...
> ```
>
> 需要注意的是, 当编译器无法判断某些场景是否一定会被初始化 或 无法判断是否重复初始化了不可变变量时, 会倾向于保守策略进行编译报错, 见如下示例:
>
> ```cangjie
> func calc(a: Int32){
>     println(a)
...
> 
> class A {
>     public static var ctx: Int64      // error: 静态成员变量 'ctx' 未初始化
> 
>     static init() {
>         let lambda1 = {=> ctx = 10}
>         foo(lambda1)
...
全局变量和静态变量(非成员), **必须在定义时进行初始化**

静态成员变量, 如果尝试在`lambda`表达式中初始化, 或另外的函数中初始化, 编译器无法判断`lambda`或相应的函数被执行, 所以会编译报错
...
对于**值类型变量**, 给值类型变量赋值, 会直接改变变量地址的数据

且, 给值类型变量赋值, 如果`=`右边是另外一个变量时, 则发生拷贝动作, 最终两个变量之间没有任何关联

对于**引用类型变量**, 给引用类型变量赋值, **只是改变了变量的引用关系**

给引用类型变量赋值, 如果`=`右边是另一个引用类型变量, 左边的变量会引用右边的变量, 两个变量最终共享同一个实例

这说明什么?

这说明 **引用类型变量 并不是实例本身, 而是实例的引用, 类似C语言的指针, 引用类型的变量 只是指向了实例, 而不是实例本身**
