...
难道这是 C 语言设计的吗? 通过`printf()`打印字符串, 打印结果会在末尾加一个特殊符号`⏎`?
...
| `'\e'`   | `27`   | 同 <Kbd>Esc</Kbd> 按键的输入(非标准 ASCII C 中的转义, GCC 等编译器可能扩展实现)                                 |
...
**在 C 语言中, `ASCII`码表中所有的字符实际上就是整型值, 需要字符形式输出的时候 表现为字符, 需要整型输出的时候 则表现为整型**
...
这意味着, **在 C 语言中, 字符的真实类型实际是整型, `char`类型只是占用`1byte`大小的整型类型**
...
这一切现象, 都说明了: **字符在 C 语言中的实际类型是整型, `char`仅表示一种特定大小的整型类型**
...
> `strlen()`
>
> C 语言标准中, 用于 取字符串有效长度的函数, 以`'\0'`判断字符串结尾
>
